...
| CHK-CI-G3 | All `uses:` entries SHA-pinned. | docs/08_SECURITY_GATES.md#security-pinning | build_proof/ci_hardening/outputs/01_action_pin_audit.txt:L1-L40; build_proof/ci_hardening/outputs/12_not_pinned_check.txt:L1-L2 | PASS |
...
| CHK-CI-G6B | `actionlint` command execution is proven (local or CI log). | docs/06_CI_AND_RELEASE.md#security-gates-hardening | build_proof/ci_hardening/outputs/07_actionlint__version.txt:L1-L3 | FAIL (`actionlint` unavailable locally; CI log retrieval blocked). |
...
| ItemID | Requirement | Doc section anchor | Evidence anchors | Status |
|---|---|---|---|---|
| CHK-CI-BLOCK-PIN-API | Upstream commit+tarball API validation for repaired failing pins (G1). | docs/08_SECURITY_GATES.md#pin-validation-api | build_proof/ci_hardening/outputs/02_pin_validation_api.txt:L1-L31; build_proof/ci_hardening/outputs/98_operator_runbook.txt:L1-L35 | FAIL (cannot call GitHub API from this environment). |
| CHK-CI-BLOCK-LOGS | Decisive per-job error excerpts captured from GitHub Actions runs. | docs/08_SECURITY_GATES.md#security-evidence-limitations | build_proof/ci_hardening/outputs/15_gh_capability_check.txt:L1-L12; build_proof/ci_hardening/outputs/16_github_api_runs_probe.txt:L1-L8 | FAIL (cannot access runs/logs from this environment). |
| CHK-CI-BLOCK-ACTIONLINT-INSTALL | Local actionlint install/execution for CHK-CI-G6B. | docs/08_SECURITY_GATES.md#security-evidence-limitations | build_proof/ci_hardening/outputs/07_actionlint__version.txt:L1-L3 | FAIL (binary unavailable). |
| CHK-CI-BLOCK-CI-PASS | Proof that all required PR checks are green for PR #100 branch. | docs/08_SECURITY_GATES.md#security-evidence-limitations | build_proof/ci_hardening/outputs/15_gh_capability_check.txt:L1-L12; build_proof/ci_hardening/outputs/16_github_api_runs_probe.txt:L1-L8; build_proof/ci_hardening/outputs/98_operator_runbook.txt:L1-L35 | FAIL (verification blocked; operator runbook required). |
