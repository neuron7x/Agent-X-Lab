...
def main() -> int:
    ...
    p_val.add_argument("--strict", action="store_true", help="Strict mode (recommended)")
    ...
    p_eval.add_argument("--write-evidence", action="store_true", help="Write evidence under objects/*/artifacts/evidence/")
    p_eval.add_argument("--deterministic", action="store_true", help="Deterministic run (writes to reference/)")

    p_schema = sub.add_parser("schema", help="Validate JSON artifacts against JSON Schemas")
    p_schema.add_argument("--repo-root", default=".", help="Repo root (default: .)")

    p_rebuild = sub.add_parser("rebuild", help="Rebuild checksums and catalog index deterministically")
    p_rebuild.add_argument("--repo-root", default=".", help="Repo root (default: .)")

    p_pack = sub.add_parser("package", help="Create a clean release zip (no runtime evidence)")
    ...
    if args.cmd == "rebuild":
        rc = subprocess.run(["python", "scripts/rebuild_checksums.py", "--repo-root", str(root)]).returncode
        if rc != 0:
            return rc
        return subprocess.run(["python", "scripts/rebuild_catalog_index.py", "--repo-root", str(root)]).returncode
        ...
        with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as z:
            for p in root.rglob("*"):
                ...
                if rel.startswith("objects/") and "/artifacts/evidence/" in rel:
                    if "/artifacts/evidence/reference/" not in rel and not rel.endswith("artifacts/evidence/.gitkeep"):
                        continue
