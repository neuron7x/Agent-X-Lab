...
def main() -> int:
    ...
    ap.add_argument("--manifest", default="MANIFEST.json", help="Manifest path relative to repo root")
    ap.add_argument("--strict", action="store_true", help="Enable strict checks (recommended for CI)")
    args = ap.parse_args()
    ...
    for k, cid in [("arsenal", "M02"), ("objects", "M03"), ("protocols", "M04"), ("architecture", "M05"), ("metrics", "M06"), ("checksums", "M07")]:
        add(checks, cid, k in manifest, f"root MANIFEST has key: {k}")

    add(checks, "M08", isinstance(manifest.get("objects", []), list), "MANIFEST.objects is a list")
    add(checks, "M09", isinstance(manifest.get("protocols", []), list), "MANIFEST.protocols is a list")
    add(checks, "M10", isinstance(manifest.get("architecture", []), list), "MANIFEST.architecture is a list")
    add(checks, "M11", isinstance(manifest.get("metrics", {}), dict), "MANIFEST.metrics is an object")
    add(checks, "M12", isinstance(manifest.get("checksums", {}), dict), "MANIFEST.checksums is an object")

    # --- Metrics cross-check
    metrics = manifest.get("metrics", {}) if isinstance(manifest.get("metrics", {}), dict) else {}
    add(checks, "X01", metrics.get("total_objects") == len(manifest.get("objects", [])), "metrics.total_objects matches objects length")
    add(checks, "X02", metrics.get("total_protocols") == len(manifest.get("protocols", [])), "metrics.total_protocols matches protocols length")
    add(checks, "X03", metrics.get("total_architecture_docs") == len(manifest.get("architecture", [])), "metrics.total_architecture_docs matches architecture length")

    # --- Referenced paths
    # Protocols
    for i, p in enumerate(manifest.get("protocols", []), start=1):
        rel = (p or {}).get("path", "")
        add(checks, f"P{i:02d}", (repo_root / rel).exists(), f"protocol path exists: {rel}")

    # Architecture
    for i, a in enumerate(manifest.get("architecture", []), start=1):
        rel = (a or {}).get("path", "")
        add(checks, f"A{i:02d}", (repo_root / rel).exists(), f"architecture path exists: {rel}")
    ...
    for i, o in enumerate(objects, start=1):
        ...
        add(checks, f"O{i:02d}", (repo_root / obj_bundle_rel).exists(), f"object IO-BUNDLE exists: {obj_name} -> {obj_bundle_rel}")
        ...
        if args.strict:
            for j, rel in enumerate(
                ...
                add(checks, f"S{i:02d}.{j:02d}", (obj_dir / rel).exists(), f"strict object path exists: {obj_name}/{rel}")

            # No runtime evidence committed (only reference + .gitkeep allowed)
            ev = obj_dir / "artifacts/evidence"
            bad = []
            if ev.exists():
                for child in ev.iterdir():
                    if child.name in {"reference", ".gitkeep"}:
                        continue
                    bad.append(child.name)
            add(checks, f"E{i:02d}", len(bad) == 0, f"no non-reference evidence dirs committed: {obj_name} bad={bad}")
            ...
            try:
                ...
                add(checks, f"V{i:02d}", ok, f"reference eval report PASS (score=100): {obj_name}")
            except Exception as e:
                add(checks, f"V{i:02d}", False, f"reference eval report parse failure: {obj_name} {e!r}")

        # --- Schemas (strict)
    if args.strict:
        p = subprocess.run(
            ["python", str(repo_root / "scripts" / "schema_validate.py"), "--repo-root", str(repo_root)],
            capture_output=True,
            text=True,
        )
        ok = p.returncode == 0
        msg = "schema_validate OK" if ok else ("schema_validate failed: " + (p.stdout or p.stderr or "")[:800])
        add(checks, "S00", ok, msg)

# --- Checksums (strict)
    if args.strict:
        csum = manifest.get("checksums", {}) if isinstance(manifest.get("checksums", {}), dict) else {}
