...
v2: graceful error handling, Result-aware, structured diagnostics.
"""
...
def _get_token() -> str:
    ...
    if not token:
        console.print("[red bold]Error:[/red bold] GITHUB_TOKEN not set")
        console.print("[dim]Export your GitHub token: export GITHUB_TOKEN=ghp_...[/dim]")
        raise typer.Exit(1)
    return token
...
def run(
    ...
    constraints = Constraints(
        ...
        refactor_policy=RefactorPolicy(cfg_data.get("refactor_policy", "no-refactor")),
        security_policy=SecurityPolicy(
            no_disable_security_checks=cfg_data.get("no_disable_security_checks", True),
            actions_must_be_pinned=cfg_data.get("actions_must_be_pinned", True),
            dependencies_must_be_pinned=cfg_data.get("dependencies_must_be_pinned", True),
        ),
    )
    ...
    with CIOracle(token, pr) as oracle:
        obs_result = oracle.observe()
        if obs_result.is_err():
            console.print(f"[red]Error:[/red] {obs_result.unwrap_err()}")
            raise typer.Exit(1)

        sha, checks = obs_result.unwrap()

        diff_result = oracle.get_diff_summary()
        if diff_result.is_err():
            console.print(f"[red]Error:[/red] {diff_result.unwrap_err()}")
            raise typer.Exit(1)
        diff_summary = diff_result.unwrap()

        files_result = oracle.get_changed_files()
        if files_result.is_err():
            console.print(f"[red]Error:[/red] {files_result.unwrap_err()}")
            raise typer.Exit(1)
        touched_files = files_result.unwrap()
...
def verify(
    ...
    checkpoint_store = CheckpointStore(store) if store else CheckpointStore()
    result = checkpoint_store.load_bundle(hash_prefix)

    if result.is_err():
        console.print(f"[red]Error loading bundle:[/red] {result.unwrap_err()}")
        raise typer.Exit(1)
    ...
    console.print(Panel(
        ...
        f"[bold]Schema:[/bold] {bundle.schema_version}\n"
...
def _render_decision(decision: GovernorDecision, loop: CanonicalLoop) -> None:
    ...
    action_styles = {
        GovernorAction.MERGE: "[bold green]MERGE[/bold green]",
        GovernorAction.HOLD:  "[bold yellow]HOLD[/bold yellow]",
        GovernorAction.NEXT:  "[bold cyan]NEXT LOOP[/bold cyan]",
        GovernorAction.HALT:  "[bold red]HALT[/bold red]",
    }

    console.print()
    console.print(Panel(
        f"[bold]Decision:[/bold] {action_styles.get(decision.action, decision.action.value)}\n"
